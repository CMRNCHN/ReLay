...
public final class SpatialTransitionEngine {
    ...
    func commitSession(effectiveX: CGFloat, effectiveY: CGFloat, fingerCount: Int, at location: CGPoint, sessionID: String) {
        ...
        if fingerCount >= 4 {
            ...
            if let dir = direction, dir == .down {
                executeLayoutExpose(triggerWindow: window, sessionID: sessionID)
            } else {
    ...
    private func executeLayoutExpose(triggerWindow: AXUIElement, sessionID: String) {
        AppLogger.log("transition request layout-expose", sessionID: sessionID, subsystem: "transition")
        PreviewManager.shared.dismiss(animated: false, sessionID: sessionID)
        LayoutExposeController.shared.present(triggerWindow: triggerWindow, sessionID: sessionID)
    }
